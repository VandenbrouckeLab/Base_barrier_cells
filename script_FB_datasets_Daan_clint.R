## Initial FB analysis with atlas datasets to check origin of FB2s
## Still contains everything and then subsetted without CPE and Imm
...
########################################
dir.create(paste0(sampleFolder,"results_merge/Heatmaps"))

# ## Load in markers
# RNAMarkers_RNAclus<- readRDS(file=paste0(sampleFolder,"results_merge/Robjects/RNAmarkersList_RNAclus_",sampleName,".rds"))
#
...
# ggsave(D1, file=paste0(sampleFolder, "results_merge/Heatmaps/Heatmap_RNAmarkersList_RNAclus_",sampleName,".png"),
#        height = 20, width = 12, dpi = "retina")
#
# pdf(file=paste0(sampleFolder, "results_merge/Heatmaps/Heatmap_RNAmarkersList_RNAclus_",sampleName,".pdf"),
#     height = 20, width = 25)
...
# RNAMarkers_RNAclus <- FindAllMarkers(seuratObj, assay = "RNA", only.pos = TRUE)
# table(RNAMarkers_RNAclus$cluster)
# saveRDS(RNAMarkers_RNAclus, file=paste0(sampleFolder,"results_merge/Robjects/RNAmarkersList_RNAclus_",sampleName,"_sliced.rds"))
# 
# ### Add to diagnostics
...
# saveRDS(diagnostics, file=paste0(sampleFolder,"results_merge/Robjects/diagnostics_sliced_",sampleName,"_clint.rds"))
# 
# ### Create list with markers
# totalNrRNAclusters_RNAclus<-max(as.numeric(names(table(RNAMarkers_RNAclus$cluster))))
# totalNrRNAclusters_RNAclusPlusOne<-totalNrRNAclusters_RNAclus+1
...
# ### Write to Excel
# library('openxlsx')
# write.xlsx(RNAmarkersList_RNAclus, file =paste0(sampleFolder, "results_merge/Marker_lists/RNAmarkersList_RNAclus_",sampleName,"_sliced.xlsx"))
...
# 
# png(file=paste0(sampleFolder,"results_merge/Annotation/3_SampleDistribution_ggplot2_annot_1_",sampleName,"_adjusted.png"), width = 2000, height = 1500, res = 300)
...
## Extra for Daan: split off CPE and Imm
...
## Extra plot for contamination Brain.Pdgfra samples (11/01/22)
Idents(seuratObjNew)<-seuratObjNew@meta.data$sample_origin2_optionB

F4 <- FeaturePlot(seuratObjNew, features = "Aqp4", order = T, cols = c("Yellow","Red"),
                  reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, split.by = "sample_origin2_optionA",
                  label = T) 
F4.5 <- FeaturePlot(seuratObj, features = "Aqp4", order = T, cols = c("Yellow","Red"),
                  reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, split.by = "sample_origin",
                  label = T) 
F5 <- FeaturePlot(seuratObjNew, features = "Mbp", order = T, cols = c("Yellow","Red"),
                  reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, split.by = "sample_origin2_optionA",
                  label = T) 

pdf(file = paste0(sampleFolder,"results_merge/Annotation/10_UMAP_Featureplot_Vanlandewijck_Aqp4_",sampleName,"_no_CPE_or_Imm.pdf"), width = 40, height = 7)
F4
dev.off()

pdf(file = paste0(sampleFolder,"results_merge/Annotation/10_UMAP_Featureplot_Vanlandewijck_Aqp4_",sampleName,".pdf"), width = 40, height = 7)
F4.5
dev.off()

pdf(file = paste0(sampleFolder,"results_merge/Annotation/10_UMAP_Featureplot_Vanlandewijck_Mbp_",sampleName,"_no_CPE_or_Imm.pdf"), width = 40, height = 7)
F5
dev.off()


## Extra plot for Pdgfra (14/02/22)
# Idents(seuratObjNew)<-seuratObjNew@meta.data$sample_origin2_optionB

F4 <- FeaturePlot(seuratObjNew, features = "Pdgfra", order = T, cols = c("Yellow","Red"),
                  reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, label = T) 
F4.5 <- FeaturePlot(seuratObjNew, features = "Pdgfra", order = T, cols = c("Yellow","Red"),
                    reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, split.by = "sample_origin2_optionA",
                    label = T) 
# V4.5 <- VlnPlot(seuratObjNew, features = "Pdgfra", split.by = "sample_origin2_optionA") 

pdf(file = paste0(sampleFolder,"results_merge/Feature_plots/Featureplot_Pdgfra_v1_",sampleName,"_no_CPE_or_Imm.pdf"), width = 9, height = 10)
F4
dev.off()

pdf(file = paste0(sampleFolder,"results_merge/Feature_plots/Featureplot_Pdgfra_v2_",sampleName,"_no_CPE_or_Imm.pdf"), width = 40, height = 7)
F4.5
dev.off()
...
pdf(file=paste0(sampleFolder,"results_merge/Feature_plots/Feature_plot_paper_Tcf21_",sampleName,"_viridisC_ordered.pdf"), height = 10, width = 8)
for (feature in features) {
  F1<-FeaturePlot(object = seuratObjNew, features =feature, cols = c("grey", "blue"), 
                  reduction = "umap", min.cutoff = 'q2', max.cutoff = 'q98', pt.size = 1.5, order=T) +
...
# Create dotplot
Idents(seuratObjNew)<-seuratObjNew$New_clusters

Colors_dotplot<-c("#071AE5","#F50635") #030720
D1<-DotPlot(seuratObjNew, features = rev(Figure_genes), cols = Colors_dotplot) + RotatedAxis()

pdf(file=paste0(sampleFolder,"results_merge/Feature_plots/Feature_plots_and_dotplots_Aqp_",sampleName,".pdf"), height = 10, width = 15)
D1
...
################################################################################
dir.create(paste0(sampleFolder,"results_merge/neededFilesOnlineTool"))
...
saveRDS(exportShiny, file=paste0(sampleFolder,"results_merge/neededFilesOnlineTool/SO_",sampleName,".rds"))
annotation <- exportShiny@meta.data$annotated_clusters %>% as.factor()
saveRDS(annotation, file=paste0(sampleFolder,"results_merge/neededFilesOnlineTool/SO_",sampleName,"_annotation.rds"))

# exportShinyNew<-DisneyTools::expShiny(seuratObjNew, conditions = unlist(listLabels), assay = "RNA")
# saveRDS(exportShinyNew, file=paste0(sampleFolder,"results_merge/neededFilesOnlineTool/SO_",sampleName,"_Pos_vs_Neg.rds"))
# annotationNew <- exportShinyNew@meta.data$newClusters %>% as.factor() 
# saveRDS(annotationNew, file=paste0(sampleFolder,"results_merge/neededFilesOnlineTool/SO_",sampleName,"_Pos_vs_Neg_annotation.rds"))
